...
def mtKMeans(dataset, K, mutex):         
    ...
    while error != 0 and i < 30:
        i += 1
        classif = mtClassification(dataset,prev_centroids,mutex)
...
Method to perform X Kmeans over the same dataset and keep the best centroids
'''    
def mpMultipleKMeans(dataframe, K, mutex, iterations=5):
    centroids = []
    list_avg = []
    for i in range(iterations):
        centroid, classif = mtKMeans(dataframe,K,mutex)
        avg_dist = avgDistance(dataframe,centroid,classif)
        centroids.append(centroid)
        list_avg.append(avg_dist)
    list_avg = np.array(list_avg)
...
def mtDistance(dataset, centroid, aux, mutex, i):
    x = np.sqrt(np.sum((dataset[:,1:]-centroid)**2, axis = 1))
    mutex.acquire()
    aux[i] = x
    mutex.release()

def mtClassification(dataset, centroids, mutex):
    m,n = dataset.shape
    classif = np.zeros(m) # probar comentar este
    aux = [i for i in range(len(centroids))]
    iterdata = [(dataset, centroids[i], aux, mutex, i) for i in range(len(centroids))] 
    ths = []
    for i, thread in enumerate(iterdata):
         th = threading.Thread(name='th%s' %(i+1), target= mtDistance, args= iterdata[i])
         th.start()
         ths.append(th)
    for thread in ths:
         thread.join()
...
if __name__ == '__main__':
    ...
    k= 4
    mutex = Lock()
    centroids, distance = mpMultipleKMeans(dataset,k,mutex)
